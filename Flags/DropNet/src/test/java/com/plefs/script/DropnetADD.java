...
public class DropnetADD extends BaseClass {
	...
	ExtentReports report = new ExtentReports("D:/TestNG_Sample/rep.html", false); 
	...
	public void reportinit() throws IOException, InterruptedException {
		...
		HTMLReportGenerator.TestSuiteStart(ReportPath + "\\" + "TestResult_Regression_" + currentdatetime + ".html",
				"OffshoreFlags");
		/*
		 * DropnetADD browser = new DropnetADD(); browser.intilaize();
		 */
	}
...

	{
		try {
			...
			if (isRunnableTest) {
				...
				// obj_common_methods.waitForElement(driver, ObjectRepository.by_Dropnetpage,
				// 30);
				...
				driver.findElement(ObjectRepository.by_Add_BuildingInfo_CmpnyCntctPhoneValue1)
						.sendKeys(building_contact_num);
				 ...
				 * driver.findElement(ObjectRepository.by_AddBttn);
...

	{
		try {
			...
			if (isRunnableTest) {
				 ...
				 * driver.findElement(ObjectRepository.by_Payee_FeeAmount).sendKeys(fee_amt);
				 * String taxidpayee = DataObject.getVariable("Tax_id", "TC_02");
				 * driver.findElement(ObjectRepository.by_Payee_TaxIDAppValue).sendKeys(
				 * taxidpayee); String taxidpayeepercent = DataObject.getVariable("Tax_id%",
				 * "TC_02");
				 * driver.findElement(ObjectRepository.by_Payee_TaxPerAppValue).sendKeys(
				 * taxidpayeepercent); String Escalation = DataObject.getVariable("Escalation",
				 * "TC_02") ;
				 * driver.findElement(ObjectRepository.by__payee_escalationPct).sendKeys(
				 * Escalation); String Contct_number_payee =
				 * DataObject.getVariable("Phone_number", "TC_02");
				 * driver.findElement(ObjectRepository.by__payee_PhoneNbr).sendKeys(
				 * Contct_number_payee); String fax_number_payee =
				 * DataObject.getVariable("FAX_Number", "TC_02");
				 * driver.findElement(ObjectRepository.by__payee_payeeFaxNbr).sendKeys(
				 * fax_number_payee); String Comments = DataObject.getVariable("Comments",
				 * "TC_02");
				 * driver.findElement(ObjectRepository.by_Payee_CommentsAppValue).sendKeys(
				 * Comments); String Contract_exp = DataObject.getVariable("Expirationdt",
				 * "TC_02");
				 * driver.findElement(ObjectRepository.by__payee_contractExpirationDt).sendKeys(
				 * Contract_exp);
				 */
				// String parentwindow = driver.getWindowHandle();
