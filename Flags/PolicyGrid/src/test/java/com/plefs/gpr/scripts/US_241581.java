...
public class US_241581 {
	Logger logger = Logger.getLogger(US_241581.class);

	/**********************************************************************************************
	 * Description: Verify that the user is able to view report status as "D" in
	 * FILE_STORAGE table if user denies report by clicking on Deny button.
			...
			if (testRunnable) {
				...
				for (int k = 1; k < 3; k++) {
					/*BrowserInitialization.driver.findElement(By.xpath(
							"(//span[text()='Approval Status']/../../../..//div[text()='C']" + "/../..//div[text()='OFFERING_DIMENSION_CONSTRAINTS']/../..//input[@type='checkbox'])[" + k + "]"))
							.click();*/
					BrowserInitialization.driver.findElement(By.xpath(
				...
				Thread.sleep(30000);
				Application.findButtonAndClick(XpathRepository.by_DenyButton);;
				Thread.sleep(10000);
				...
				if (result.contentEquals("D")) {
					...
					Reporter.setTestDetails("PASS", testCaseDesc, failTestCaseDesc, screenshotPath);
				} else {
